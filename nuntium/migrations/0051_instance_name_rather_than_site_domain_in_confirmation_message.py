...
class Migration(DataMigration):
    """Fix up confirmation email templates.

    Replace the unusable {{ site }} base template variable
    with the name of the instance from {{ confirmation }}.
    """

    old_template_re = r'{{\s*site.current_site\.domain\s*}}'
    new_template_var = '{{ confirmation.message.writeitinstance.name }}'

    def forwards(self, orm):
        for template_object in orm.ConfirmationTemplate.objects.all():
            template_object.content_html = re.sub(
                self.old_template_re,
                self.new_template_var,
                template_object.content_html,
                )
            template_object.content_text = re.sub(
                self.old_template_re,
                self.new_template_var,
                template_object.content_text,
                )
    ...
    def backwards(self, orm):
        # I can see no reason to reverse this
        pass
    ...
    models = {
        ...
        u'nuntium.confirmation': {
            ...
            'created': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2015, 3, 3, 0, 0)'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        u'nuntium.messagerecord': {
            ...
            'content_type': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['contenttypes.ContentType']"}),
            'datetime': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2015, 3, 3, 0, 0)'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
