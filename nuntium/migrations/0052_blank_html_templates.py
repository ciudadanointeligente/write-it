...
default_new_answer_html = """Dear {{user}}: <br/>
<br/>
...
default_confirmation_html = """Hello {{ confirmation.message.author_name }}:<br />
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for instance in orm.WriteItInstance.objects.all():
            ...
            if whitespace_ignoring_equal(default_new_answer_html, new_answer_notification_template.template_html):
                new_answer_notification_template.template_html = ''
                new_answer_notification_template.save()
            else:
                print "MANUAL UPDATE NEEDED: {} {} has modified new answer html - ignoring".format(instance.id, instance.name)

            if whitespace_ignoring_equal(confirmation_template.content_html, default_confirmation_html):
                confirmation_template.content_html = ''
                confirmation_template.save()
            else:
                print "MANUAL UPDATE NEEDED: {} {} has modified confirmation html - ignoring".format(instance.id, instance.name)
    ...
    def backwards(self, orm):
        for instance in orm.WriteItInstance.objects.all():
            ...
            if not new_answer_notification_template.template_html:
                new_answer_notification_template.template_html = default_new_answer_html
                new_answer_notification_template.save()

            if not confirmation_template.content_html:
                confirmation_template.content_html = default_confirmation_html
                confirmation_template.save()
    ...
    models = {
        ...
        u'nuntium.confirmation': {
            ...
            'created': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2015, 3, 10, 0, 0)'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        u'nuntium.messagerecord': {
            ...
            'content_type': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['contenttypes.ContentType']"}),
            'datetime': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2015, 3, 10, 0, 0)'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
