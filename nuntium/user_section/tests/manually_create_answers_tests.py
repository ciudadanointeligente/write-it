...
class ManuallyCreateAnswersTestCase(UserSectionTestCase):
    ...
    def test_it_can_be_reached(self):
        ...
        self.assertIn('writeit_messages', response.context)
        self.assertEquals(len(response.context['writeit_messages']), self.writeitinstance.message_set.count())
        self.assertIn(self.writeitinstance.message_set.all()[0], response.context['writeit_messages'])
        self.assertIn(self.writeitinstance.message_set.all()[1], response.context['writeit_messages'])
        self.assertIn(self.writeitinstance.message_set.all()[2], response.context['writeit_messages'])
