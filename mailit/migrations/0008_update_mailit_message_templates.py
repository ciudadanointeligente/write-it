...
old_mailit_subject = "[WriteIT] Message: %(subject)s"

class Migration(DataMigration):

    def forwards(self, orm):
        new_mailit_text = self.models['mailit.mailittemplate']['content_template'][2]['default']
        new_mailit_subject = self.models['mailit.mailittemplate']['subject_template'][2]['default']
        ...
        for template in orm.MailItTemplate.objects.all():
            ...
            if whitespace_ignoring_equal(old_mailit_subject, template.subject_template):
                template.subject_template = new_mailit_subject
                template.save()
            else:
                print "MANUAL UPDATE NEEDED: {} {} has modified mailit subject - ignoring".format(template.writeitinstance.id, template.writeitinstance.name)
    ...
    def backwards(self, orm):
        ...
        for template in orm.MailItTemplate.objects.all():
            ...
            if whitespace_ignoring_equal(new_mailit_subject, template.subject_template):
                template.subject_template = old_mailit_subject
                template.save()
            else:
                print "MANUAL UPDATE NEEDED: {} {} has modified mailit subject - ignoring".format(template.writeitinstance.id, template.writeitinstance.name)
